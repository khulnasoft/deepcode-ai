...
A demo is also available on the [🤗 Hugging Face Space](https://huggingface.co/spaces/deepcode-ai/deepcode-ai-33b-instruct), and you can run the demo locally using `app.py` in the [demo](https://github.com/deepcode-ai/deepcode-ai/tree/main/demo) folder. (Thanks to all the HF team for their support)
...
import torch
tokenizer = AutoTokenizer.from_pretrained("deepcode-ai/deepcode-ai-6.7b-base", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("deepcode-ai/deepcode-ai-6.7b-base", trust_remote_code=True, torch_dtype=torch.bfloat16).cuda()
...
from transformers import AutoTokenizer, AutoModelForCausalLM
import torch
tokenizer = AutoTokenizer.from_pretrained("deepcode-ai/deepcode-ai-6.7b-instruct", trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("deepcode-ai/deepcode-ai-6.7b-instruct", trust_remote_code=True, torch_dtype=torch.bfloat16).cuda()
messages=[
...
```
You are an AI programming assistant, utilizing the DeepCode Coder model, developed by DeepCode Company, and you only answer questions related to computer science. For politically sensitive questions, security and privacy issues, and other non-computer science questions, you will refuse to answer.
### Instruction:
['content']
### Response:
['content']
...
We provide script `finetune/finetune_deepcodeai.py` for users to finetune our models on downstream tasks.
...
After data preparation, you can use the sample shell script to finetune `deepcode-ai/deepcode-ai-6.7b-instruct`.
...
MODEL="deepcode-ai/deepcode-ai-6.7b-instruct"

cd finetune && deepspeed finetune_deepcodeai.py \
...
model_name = "deepcode-ai/deepcode-ai-6.7b-base"
...
tp_size = 4 # Tensor Parallelism
sampling_params = SamplingParams(temperature=0.7, top_p=0.9, max_tokens=100)
model_name = "deepcode-ai/deepcode-ai-6.7b-instruct"
...
# generate GGUF model
python convert-hf-to-gguf.py <MODEL_PATH> --outfile <GGUF_PATH> --model-name deepcodeai
...
#### How to use the deepcode-ai-instruct to complete the code?

Although the deepcode-ai-instruct models are not specifically trained for code completion tasks during supervised fine-tuning (SFT), they retain the capability to perform code completion effectively. To enable this functionality, you simply need to adjust the eos_token_id parameter. Set the eos_token_id to 32014, as opposed to its default value of 32021 in the deepcode-ai-instruct configuration. This modification prompts the model to recognize the end of a sequence differently, thereby facilitating code completion tasks.

### 8. Resources

[awesome-deepcode-ai](https://github.com/deepcode-ai/awesome-deepcode-ai) is a curated list of open-source projects related to DeepCode Coder.

### 9. License

This code repository is licensed under the MIT License. The use of DeepCode Coder models is subject to the Model License. DeepCode Coder supports commercial use.
...
@misc{deepcode-ai,
  ...
  title = {DeepCode-AI: When the Large Language Model Meets Programming -- The Rise of Code Intelligence},
  journal = {CoRR},
